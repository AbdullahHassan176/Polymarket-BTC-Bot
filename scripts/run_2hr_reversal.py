"""
Run a 2-hour paper test: reversal strategy only.

- STRATEGY_MODE = "reversal"
- Buy when YES or NO <= 10c; sell when bid >= 50c (mean reversion).
- Trades log: logs/trades_2hr_reversal.csv
- Creates STOP_BOT.txt after 2 hours.

Run: python run_2hr_reversal.py
"""
...
execution.set_trades_csv(os.path.join("logs", "trades_2hr_reversal.csv"))
...
def _reset_state() -> None:
    """Reset state.json for fresh 2hr reversal run."""
    ...
    risk.state = state
    risk._save()
    logger.info("State reset for 2hr reversal paper test.")
...
def main() -> None:
    ...
    logger.info("=" * 60)
    logger.info("2-HOUR PAPER TEST: reversal (buy <=10c, TP at 50c)")
    logger.info("Trades log: logs/trades_2hr_reversal.csv")
